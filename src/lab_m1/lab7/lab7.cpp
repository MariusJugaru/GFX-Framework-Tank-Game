...
{
    {
        ...
        RenderSimpleMesh(meshes["sphere"], shaders["LabShader"], modelMatrix, glm::vec3(0.5, 0.75, 1));
    }
    ...
    // Render ground
    {
        ...
        RenderSimpleMesh(meshes["plane"], shaders["LabShader"], modelMatrix, glm::vec3(1, 0.5, 1));
    }
...
void Lab7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    int kd_position = glGetUniformLocation(shader->program, "material_kd");
    glUniform1f(kd_position, materialKd);

    int ks_position = glGetUniformLocation(shader->program, "material_ks");
    glUniform1f(ks_position, materialKs);

    int color_position = glGetUniformLocation(shader->program, "object_color");
    glUniform3fv(color_position, 1, glm::value_ptr(color));
