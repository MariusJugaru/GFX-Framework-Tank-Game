...
void Lab9::Update(float deltaTimeSeconds)
{
...

    {
        ...
        modelMatrix = glm::rotate(modelMatrix, glm::atan2(0 - GetSceneCamera()->m_transform->GetWorldPosition().x, 0 - GetSceneCamera()->m_transform->GetWorldPosition().z), glm::vec3(0, 1, 0));
...
void Lab9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
    ...
    // TODO(student): Set any other shader uniforms that yo u need
    float time_location = glGetUniformLocation(shader->program, "time");
    glUniform1f(time_location, Engine::GetElapsedTime());

    glUniform1i(glGetUniformLocation(shader->program, "texture_id"), texture1->GetTextureID());
